...
grep -q '^memory-limit=' server.properties && sed -i 's/^memory-limit=.*/memory-limit=$server_memory/' server.properties || echo 'memory-limit=$server_memory' >> server.properties
